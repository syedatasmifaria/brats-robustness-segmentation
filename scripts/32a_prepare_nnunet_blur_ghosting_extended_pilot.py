#!/usr/bin/env python3
"""
Script 32A: Prepare nnU-Net blur and ghosting extended pilot inputs.

Purpose:
- Extend blur and ghosting from L1-L5 to provisional L6-L10.
- Use five held-out test patients as a pilot.
...
- The candidate L6-L10 parameters are provisional.
...
# Provisional candidate levels.
# These must pass visual and MSE/PSNR checks before full evaluation.
EXTENDED_LEVELS = {
    "blur": {
        6: {"sigma": 3.0},
        7: {"sigma": 3.5},
...
def prepare_shared_labels(df):
    """Save one shared label file per pilot patient."""
...
def main():
    print("=" * 80)
    print("Script 32A: Prepare blur and ghosting extended pilot")
    print("=" * 80)
    ...
    for patient_id in pilot_df["patient_id"]:
        print(f"  {patient_id}")
    print()
    ...
    summary_csv = (
        OUT_ROOT
        / "32a_blur_ghosting_extended_pilot_summary.csv"
    )
    summary_txt = (
        OUT_ROOT
        / "32a_blur_ghosting_extended_pilot_summary.txt"
    )
    ...
    with open(summary_txt, "w", encoding="utf-8") as file:
        file.write("=" * 80 + "\n")
        file.write(
            "Script 32A: Blur and ghosting extended pilot summary\n"
        )
        file.write("=" * 80 + "\n\n")
        file.write(f"Pilot patients: {len(pilot_df)}\n")
    ...
    print()
    print("=" * 80)
    print("Blur and ghosting extended pilot preparation complete.")
    ...
    print("Next: verify the script and then prepare pilot images.")
