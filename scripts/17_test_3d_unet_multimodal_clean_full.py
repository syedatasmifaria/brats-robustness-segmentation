...
class ConvBlock3D(nn.Module):
    """
    Two 3D convolution layers.

    This matches Script 16 checkpoint naming:
    input_conv.block.0
    input_conv.block.1
    input_conv.block.3
    input_conv.block.4
...
class DownBlock3D(nn.Module):
    """
    MaxPool3d followed by ConvBlock3D.

    This matches checkpoint names like:
    down1.block.1.block.0.weight
    """
    def __init__(self, in_channels, out_channels):
        super().__init__()
...
class UpBlock3D(nn.Module):
    """
    ConvTranspose3d followed by skip connection concatenation and ConvBlock3D.

    Important:
    Script 16 used up1 as the deepest decoder block, then up2, then up3.
    """
    def __init__(self, in_channels, out_channels):
        super().__init__()
    ...
    def forward(self, x, skip):
        ...
        # Safety handling in case dimensions differ by 1 voxel.
        if x.shape[2:] != skip.shape[2:]:
            min_x = min(x.shape[2], skip.shape[2])
            min_y = min(x.shape[3], skip.shape[3])
            min_z = min(x.shape[4], skip.shape[4])

            x = x[:, :, :min_x, :min_y, :min_z]
            skip = skip[:, :, :min_x, :min_y, :min_z]
...
class UNet3D(nn.Module):
    """
    4-modal 3D U-Net matching Script 16 checkpoint.
    ...
    def __init__(self, in_channels=4, out_channels=4, base_channels=16):
        ...
        # IMPORTANT: These names/order match Script 16 checkpoint.
