#!/usr/bin/env python3
"""
Script 26D: Evaluate final full nnU-Net degraded predictions.

Purpose:
- Evaluate clean-trained nnU-Net on degraded full test set.
- Full degraded evaluation = 74 patients x 5 artifacts x 5 levels.
...
OUT_METRICS_CSV = RESULTS_DIR / "26d_nnunet_degraded_final_full_metrics.csv"
OUT_SUMMARY_CSV = REPORT_DIR / "26d_nnunet_degraded_final_full_summary.csv"
OUT_SUMMARY_TXT = REPORT_DIR / "26d_nnunet_degraded_final_full_summary.txt"
...
def main():
    print("=" * 80)
    print("Script 26D: Evaluate final full nnU-Net degraded predictions")
    print("=" * 80)
    ...
    for condition_dir in condition_dirs:
        ...
        if len(pred_files) != 74:
            raise ValueError(
                f"Expected 74 predictions for {condition}, found {len(pred_files)}"
            )
    ...
    with open(OUT_SUMMARY_TXT, "w") as f:
        f.write("nnU-Net final full degraded evaluation summary\n")
        f.write("=" * 80 + "\n\n")
        f.write("Model: clean-trained nnU-Net v2 3d_fullres fold 0\n")
        f.write("Evaluation: full-volume degraded test images only, no retraining\n")
        f.write("Patients: 74\n")
        ...
        f.write("Mean metrics by condition:\n")
        f.write("-" * 80 + "\n")
        ...
        for _, row in table.iterrows():
            f.write(
                ...
                f"TC IoU={row['iou_TC_mean']:.6f}, "
                f"ET Dice={row['dice_ET_mean']:.6f}, "
                f"ET IoU={row['iou_ET_mean']:.6f}, "
                f"Pred WT voxels={row['pred_voxels_WT_mean']:.1f}, "
                f"True WT voxels={row['true_voxels_WT_mean']:.1f}\n"
            )

    print()
    print("=" * 80)
    print("Final degraded evaluation complete.")
