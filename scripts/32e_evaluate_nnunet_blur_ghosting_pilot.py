#!/usr/bin/env python3
"""
Script 32E: Evaluate nnU-Net extended degradation pilot predictions.
...
EXTENDED_ROOT = PROJECT_ROOT / "nnunet/temporary_degraded_tests/blur_ghosting_extended_pilot"
...
OUT_METRICS_CSV = RESULTS_DIR / "32e_nnunet_blur_ghosting_pilot_metrics.csv"
OUT_SUMMARY_CSV = REPORT_DIR / "32e_nnunet_blur_ghosting_pilot_drop_summary.csv"
OUT_SUMMARY_TXT = REPORT_DIR / "32e_nnunet_blur_ghosting_pilot_drop_summary.txt"

WT_DICE_CURVE = REPORT_DIR / "32e_nnunet_blur_ghosting_pilot_wt_dice_curve.png"
WT_IOU_CURVE = REPORT_DIR / "32e_nnunet_blur_ghosting_pilot_wt_iou_curve.png"
WT_DICE_DROP_CURVE = REPORT_DIR / "32e_nnunet_blur_ghosting_pilot_wt_dice_drop_curve.png"
WT_IOU_DROP_CURVE = REPORT_DIR / "32e_nnunet_blur_ghosting_pilot_wt_iou_drop_curve.png"
...
def main():
    print("=" * 80)
    print("Script 32E: Evaluate nnU-Net extended degradation pilot")
    print("=" * 80)
    ...
    if len(pilot_case_ids) != 5:
        raise RuntimeError(f"Expected 5 pilot labels, found {len(pilot_case_ids)}")

    clean_pilot_df = clean_df[clean_df["case_id"].isin(pilot_case_ids)].copy()

    if len(clean_pilot_df) != 5:
        raise RuntimeError(
            f"Expected 5 clean baseline rows for pilot patients, found {len(clean_pilot_df)}"
        )
    ...
    for artifact in ARTIFACT_ORDER:
        for level in LEVEL_ORDER:
            ...
            if len(common_cases) != 5:
                raise RuntimeError(f"Expected 5 matched cases for {condition}, found {len(common_cases)}")
    ...
    with open(OUT_SUMMARY_TXT, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("Script 32E: nnU-Net extended pilot drop summary\n")
        f.write("=" * 80 + "\n\n")
        ...
        if len(broken) == 0:
            f.write("No extended pilot condition reached the suggested WT breaking threshold.\n")
        else:
        ...
        f.write("\nInterpretation note:\n")
        f.write(
            "This is a 5-patient pilot stress test. It should be used to decide "
    ...
    make_curve(
        ...
        title="nnU-Net extended pilot WT Dice",
        ylabel="WT Dice",
    )
    ...
    make_curve(
        ...
        title="nnU-Net extended pilot WT IoU",
        ylabel="WT IoU",
    )
    ...
    make_drop_curve(
        ...
        title="nnU-Net extended pilot WT Dice drop",
    ...
    make_drop_curve(
        ...
        title="nnU-Net extended pilot WT IoU drop",
        ylabel="WT IoU drop from clean pilot",
    )

    print("=" * 80)
    print("Extended pilot evaluation complete.")
