...
def normalize_modality(volume):
    """
    Match Script 16 training preprocessing exactly:
    z-score normalization using nonzero brain voxels.
...
def compute_patient_metrics(pred, true):
    ...
    true_tc = np.isin(true, [1, 3])

    metrics["tumor_core_dice"] = dice_binary(pred_tc, true_tc)
    metrics["tumor_core_iou"] = iou_binary(pred_tc, true_tc)

    # Enhancing tumor: label 3
    pred_et = pred == 3
    true_et = true == 3

    metrics["enhancing_tumor_dice"] = dice_binary(pred_et, true_et)
    metrics["enhancing_tumor_iou"] = iou_binary(pred_et, true_et)
...
def main():
    print("=" * 80)
    print("Script 24B: Corrected full-volume clean evaluation for custom 4-modal 3D U-Net")
    print("=" * 80)
    ...
    with open(REPORT_SUMMARY_TXT, "w") as f:
        f.write("Script 24B: Corrected full-volume clean evaluation for custom 4-modal 3D U-Net\n")
        f.write("=" * 80 + "\n\n")
